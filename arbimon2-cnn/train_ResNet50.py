...
train_dir = './data/spectrograms/'  # directory with examples of true-positive spectrograms of each class
...
gs = train_all.groupby("recording_id")
...
for c, i in enumerate(sorted(os.listdir(train_dir))):
    class_dict[c] = i

    if not i.endswith("png"):
        continue

    files.append(train_dir+'/'+i)
    tmp = np.zeros(num_classes)
    group = gs.get_group(i[:-4])
